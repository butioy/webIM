...
;!function(win, xxim, undefined){
    ...
    var config = {
        ...
        api: {
            ...
            message: 'message.json', //未读消息接口
            onlineCall:null, //设置在线/离线状态回调方法
            sendCall: null, //发送消息方法
            sendurl: '' //发送消息接口
        },
        user: { //当前用户信息
            code: '',
    ...
    DEFAULT_IMG = "/static/chatlib/layim/images/1.png";
    //xxim = {};
    ...
    xxim.popchatbox = function(othis){
        ...
        xxim.getMessage({to:config.user.code,from:dataId,type:param.type});
        $("#count_"+dataId).text("");
    };
    ...
    xxim.getGroups = function(param){
        ...
        config.json(config.api.groups, {"groupId":param.id}, function(datas){
            if(datas.status === 1){
                var ii = 0, lens = datas.data.length;
                if(lens > 0){
                    for(; ii < lens; ii++){
                        str += '<li data-id="'+ datas.data[ii].id +'" type="one"><img src="'+ (datas.data[ii].face == null ? DEFAULT_IMG : datas.data[ii].face) +'"><span class="xxim_onename">'+ datas.data[ii].name +'</span></li>';
                    }
                } else {
    ...
    xxim.transmit = function(){
        ...
        log.send = function(){
            var data = {
                ...
                //id: xxim.nowchat.id, //当前聊天对象
                from:config.user.code, //消息发送者Code码
                to:xxim.nowchat.id, //消息接收者Code码
                sign_key: '', //密匙
                type:xxim.nowchat.type,//消息类型
                _: +new Date
            };
            ...
            if(data.content.replace(/\s/g, '') === ''){
                ...
                node.imwrite.val('').focus();
                if( typeof(config.api.sendCall) == "function" ) {
                    config.api.sendCall.call(this, data);
                }
            }
        };
    ...
    xxim.event = function(){
        ...
        node.setonline.find('span').on('click', function(e){
            ...
            node.setonline.hide();
            config.api.onlineCall.call(this, {type:type,from:config.user.code});
        });
                    ...
                    if(index !== 2){
                        for(; i < myflen; i++){
                            ...
                            for(var j = 0; j < item.length; j++){
                                str += '<li data-id="'+ item[j].id +'" class="xxim_childnode" type="'+ (index === 0 ? 'one' : 'group') +'"><img src="'+ (item[j].face == null ? DEFAULT_IMG : item[j].face) +'" class="xxim_oneface"><span class="xxim_onename">'+ item[j].name +'</span>' +
                                    '<div id="count_'+item[j].id+'" class="badge pull-right" style="margin-top:5px; background-color: #FF0000;"></div></li>';
                            }
                            str += '</ul></li>';
                        }
                    } else {
    ...
    xxim.append = function( message ) {
        var log={};
        //此处皆为模拟
        var keys = xxim.nowchat.type + xxim.nowchat.id;
        //聊天模版
        log.html = function(param, type){
            return '<li class="'+ (type === 'me' ? 'layim_chateme' : 'layim_chate') +'">'
                +'<div class="layim_chatuser">'
                + function(){
                    if(type === 'me'){
                        return '<span class="layim_chattime">'+ param.time +'</span>'
                            +'<span class="layim_chatname">'+ param.name +'</span>'
                            +'<img src="'+ (param.face == null ? DEFAULT_IMG : param.face) +'" >';
                    } else {
                        return '<img src="'+ (param.face == null ? DEFAULT_IMG : param.face) +'" >'
                            +'<span class="layim_chatname">'+ param.name +'</span>'
        ...
        log.imarea.append(log.html(message, type));
        log.imarea.scrollTop(log.imarea[0].scrollHeight);
    };

    xxim.getMessage = function( param ) {
        config.json(config.api.message, param, function(resp){
            if( resp.status == 1 ) {
