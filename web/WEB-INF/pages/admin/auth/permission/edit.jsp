          ...
          <div class="col-md-12 form-horizontal">
            <form class="form-horizontal" id="edit_permission_form">
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" class="form-control" id="name" name="name" value="${permission.name}" placeholder="权限名称"/>
                </div>
              </div>
              <div class="form-group">
                <label for="name" class="col-md-4 control-label">连接地址</label>
                <div class="col-md-8">
                  <input type="text" class="form-control" id="url" name="url" value="${permission.url}" placeholder="请输入权限连接地址"/>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <select class="form-control" name="depId" id="depId" placeholder="请选择部门">
                    ...
                    <c:forEach items="${departments}" var="item">
                      <option value="${item.id}" ${permission.depId eq item.id ? 'selected' : ''}>--${item.name}--</option>
                    </c:forEach>
                  </select>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <select class="form-control" name="perGroupId" id="perGroupId" placeholder="请选择权限组">
                    <option value="">--请选择--</option>
                    <c:forEach items="${permissionGroups}" var="item">
                      <option value="${item.k}" ${item.k eq permission.perGroupId ? 'selected' : ''}>--${item.v}--</option>
                    </c:forEach>
                  </select>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <label>
                    <input type="radio" name="type" id="type_1" class="form-control" value="1" ${permission.type eq 1 ? 'checked' : ''}>&nbsp;&nbsp;菜单权限
                  </label>
                  <label style="margin-left: 30px;">
                    <input type="radio" name="type" id="type_2" class="form-control" value="2" ${permission.type eq 2 ? 'checked' : ''}>&nbsp;&nbsp;操作权限
                  </label>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <label>
                    <input type="radio" name="status" id="status_1" class="form-control" value="1" ${permission.status eq 1 ? 'checked' : ''}>&nbsp;&nbsp;启用
                  </label>
                  <label style="margin-left: 30px;">
                    <input type="radio" name="status" id="status_0" class="form-control" value="0" ${permission.status eq 0 ? 'checked' : ''}>&nbsp;&nbsp;禁用
                  </label>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <textarea name="remark" id="remark" rows="3" class="form-control" placeholder="请输入">${permisson.remark}</textarea>
                </div>
              </div>
            </form>
          </div>
    ...
    <script>
      $(function() {
        ...
        $("#submitBtn").on("click", function() {
          ...
          var param = $("#edit_permission_form").serialize();
          $.ajax({
            type:"post",
            url: "${ctx}/admin/auth/permission/doEdit",
            data: param,
            dataType:"json",
